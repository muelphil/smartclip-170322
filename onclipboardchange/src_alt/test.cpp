...
static HRESULT SetOrRefreshWindowsHook() {
    try {
        if (!g_hwnd) {
            ...
            wndClass.lpszClassName = TEXT("SmartClipListenerAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            ...
            g_hwnd = CreateWindowEx(
                0, wndClass.lpszClassName, "ClipboardChangeListenerGLEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", 0, 0, 0, 0, 0, NULL, NULL, NULL, NULL);
            if (!g_hwnd) {
                DWORD dwLastError = GetLastError();
...
int startListeningForCbc(){
    std::thread([]{
        ...
        MSG msg;
        while(!g_stopped && GetMessage(&msg, g_hwnd, 0, 0)) {
